...
import { updateBlocklyPalette } from './services/blocklySetup';
...
const App: React.FC = () => {
  ...
  const handleRun = useCallback(async () => {
    ...
    try { 
        ...
        if (e.message !== "Simulation aborted") { setIsRunning(false); } 
    }
    ...
    if (isRunning) { 
      interval = setInterval(() => { 
        ...
        if (Math.abs(currentTilt) > 3) {
            let tiltFactor = Math.min(Math.abs(currentTilt) / 25, 1);
            let reductionMultiplier = 1;
            if (fV_raw > 0 && currentTilt > 0) reductionMultiplier = Math.max(0.2, 1 - tiltFactor * 0.8);
            else if (fV_raw < 0 && currentTilt < 0) reductionMultiplier = Math.max(0.2, 1 - tiltFactor * 0.8);
        ...
        if (next.penDown) { 
          ...
          setActiveDrawing(prevActiveDrawing => {
              ...
              if (!drawingToModify || drawingToModify.color !== next.penColor) {
                  if (drawingToModify) setCompletedDrawings(oldCompleted => [...oldCompleted, drawingToModify!]);
  ...
  const orbitControlsProps = useMemo(() => {
    ...
    if (editorTool === 'PAN') {
      props.enablePan = true; props.enableRotate = false;
      props.mouseButtons = { LEFT: THREE.MOUSE.PAN, MIDDLE: THREE.MOUSE.DOLLY, RIGHT: THREE.MOUSE.ROTATE };
    } else if (editorTool === 'ROBOT_MOVE') {
      props.enablePan = false; props.enableRotate = false;
    }
    if (cameraMode === 'TOP') {
      props.enableRotate = false; props.minPolarAngle = 0; props.maxPolarAngle = 0;    
      props.mouseButtons = { LEFT: THREE.MOUSE.PAN, MIDDLE: THREE.MOUSE.DOLLY, RIGHT: THREE.MOUSE.DOLLY };
    } else if (cameraMode === 'FOLLOW') {
      props.enableRotate = false; props.enablePan = false;    
      props.minPolarAngle = Math.PI / 6; props.maxPolarAngle = Math.PI / 2 - 0.1; 
      props.mouseButtons = { LEFT: THREE.MOUSE.DOLLY, MIDDLE: THREE.MOUSE.DOLLY, RIGHT: THREE.MOUSE.DOLLY };
    }
    return props;
  ...
  useEffect(() => {
    if (controlsRef.current) {
      if (cameraMode === 'HOME') {
        controlsRef.current.reset(); controlsRef.current.minDistance = 1.2; controlsRef.current.maxDistance = 60;
      } else if (cameraMode === 'TOP') {
        controlsRef.current.object.position.set(0, 20, 0); controlsRef.current.target.set(0, 0, 0);
        controlsRef.current.minDistance = 0.1; controlsRef.current.maxDistance = 100;
      } else if (cameraMode === 'FOLLOW') {
        controlsRef.current.minDistance = 1; controlsRef.current.maxDistance = 20;
      }
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col border-4 border-slate-200">
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-slate-100">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                >
                  <h3 className={`font-bold text-lg z-10 transition-colors ${activeChallenge === null ? 'text-blue-600' : 'text-slate-800 group-hover:text-blue-600'}`}>נהיגה חופשית (ללא משימה)</h3>
                ...
                {CHALLENGES.map((challenge) => (
                  ...
                  >
                    <h3 className={`font-bold text-lg z-10 transition-colors ${activeChallenge?.id === challenge.id ? 'text-yellow-600' : 'text-slate-800 group-hover:text-blue-600'}`}>{challenge.title}</h3>
