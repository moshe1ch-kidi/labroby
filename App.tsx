...
const getSurfaceHeightAt = (qx: number, qz: number, customObjects: CustomObject[]) => {
    let maxHeight = 0;
    for (const obj of customObjects) {
        if (obj.type === 'RAMP') {
            const dx = qx - obj.x; const dz = qz - obj.z;
            ...
            if (Math.abs(lx) <= hW && Math.abs(lz) <= hL) {
                ...
                const uphillEnd = -hL + section; const downhillStart = hL - section;
                let currentY = 0;
...
const calculateSensorReadings = (x: number, z: number, rotation: number, challengeId?: string, customObjects: CustomObject[] = [], svgConfig?: any) => {
    ...
    const checkHit = (px: number, pz: number) => {
        ...
        if (['c10', 'c16', 'c19', 'c20'].includes(challengeId || '')) {
            if (Math.abs(px) <= 3 && Math.abs(pz - (-10)) <= 0.45) return true;
        }
        return false;
    };
    
    const wheelOffsetX = 0.95; const wheelOffsetZ = 0.5; const casterOffsetZ = -0.8;
    const hLeft = getSurfaceHeightAt(getPointWorldPos(-wheelOffsetX, wheelOffsetZ).wx, getPointWorldPos(-wheelOffsetX, wheelOffsetZ).wz, customObjects);
    const hRight = getSurfaceHeightAt(getPointWorldPos(wheelOffsetX, wheelOffsetZ).wx, getPointWorldPos(wheelOffsetX, wheelOffsetZ).wz, customObjects);
    const hBack = getSurfaceHeightAt(getPointWorldPos(0, casterOffsetZ).wx, getPointWorldPos(0, casterOffsetZ).wz, customObjects);
    ...
    const physicalHit = checkHit(getPointWorldPos(0, PHYSICS_RADIUS).wx, getPointWorldPos(0, PHYSICS_RADIUS).wz);
    const isTouching = checkHit(getPointWorldPos(0, TOUCH_RADIUS).wx, getPointWorldPos(0, TOUCH_RADIUS).wz);
...
const App: React.FC = () => {
  ...
  const [svgConfig, setSvgConfig] = useState<any>(undefined);
  ...
  const robotRef = useRef<RobotState>({ 
    x: 0, y: 0, z: 0, rotation: 0, tilt: 0, roll: 0, speed: 100, 
  ...
  const handleGroundPointerMove = useCallback((e: ThreeEvent<PointerEvent>) => {
    if (isDraggingRobot && editorTool === 'ROBOT_MOVE') {
      const snap = 0.5;
  ...
  const handleGroundPointerUp = useCallback((e: ThreeEvent<PointerEvent>) => {
    if (isDraggingRobot) {
      (e.target as HTMLCanvasElement).releasePointerCapture(e.pointerId);
      setIsDraggingRobot(false);
      setHomePosition({ x: robotRef.current.x, z: robotRef.current.z });
      ...
      {showChallenges && (
        <div className="fixed inset-0 z-[1000000] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col border-4 border-slate-200">
            <div className="p-6 border-b flex justify-between items-center bg-slate-50">
              <h2 className="text-2xl font-black text-slate-800 flex items-center gap-3"><Trophy className="text-yellow-500" /> Coding Challenges</h2>
